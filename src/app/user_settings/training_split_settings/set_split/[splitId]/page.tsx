...
export default function EditSplitPage({ params }: { params: { splitId: string } }) {
  ...
  // Load the existing split data when the component mounts
  useEffect(() => {
    let isMounted = true;
    ...
    async function loadSplit() {
      ...
      try {
        ...
        // If user is not available yet, wait a bit longer
        authCheckTimeout = setTimeout(async () => {
          ...
          if (user?.id) {
            ...
            try {
              ...
              if (session?.user?.id) {
                await fetchSplitData(session.user.id);
              } else {
                setError('You must be logged in to edit a split');
                setIsLoading(false);
              }
            } catch (err) {
              console.error('Error checking session:', err);
              setError('Failed to verify authentication. Please refresh the page or log in again.');
              setIsLoading(false);
            }
          }
        ...
        setError(err instanceof Error ? err.message : 'An error occurred while loading your split');
        setIsLoading(false);
      }
    }
    
    async function fetchSplitData(userId: string) {
      if (!isMounted) return;
      ...
      try {
        ...
        console.log('Fetching split data for user:', userId, 'splitId:', splitId);
        const result = await getSplitById(splitId, userId);
        
        if (!result.success) {
          throw new Error(result.error);
        }
        
        // Set form values from loaded split data
        methods.reset(result.data);
        setIsLoading(false);
      } catch (err) {
        if (!isMounted) return;
        console.error('Error in fetchSplitData:', err);
  ...
  const handleConfirmSave = async () => {
    if (!user?.id) {
      setError('You must be logged in to update a split')
      return
    }
    
    if (!splitId) {
      setError('Invalid split ID')
      return
    }
    ...
    try {
      ...
      const result = await updateSplit(formData, user.id, splitId)
      console.log('Update split result:', result)
      ...
      setError(err instanceof Error ? err.message : 'An error occurred while updating your split')
  ...
  // Show loading state while fetching the split data
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#121212] flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="h-10 w-10 animate-spin text-[#FF5733] mx-auto" />
          <p className="mt-4 text-[#b3b3b3] text-lg">Loading split data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#121212]">
      ...
      <main className="mx-auto max-w-4xl p-4 md:p-6">
        ...
        <div className="mb-6 text-center">
          <h1 className="text-3xl font-bold text-white">Edit Training Split</h1>
          <p className="mt-2 text-[#b3b3b3]">Modify your existing training split</p>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-full max-w-md rounded-lg border border-[#404040] bg-[#1e1e1e] p-6 shadow-lg">
              ...
              <p className="mb-6 text-[#b3b3b3]">
                Are you sure you want to update this training split? This will modify your existing split with the new configuration.
              </p>
